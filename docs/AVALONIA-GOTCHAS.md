...
`KeyNotFoundException` as its template loads, or renders parts of the editor and its search panel
invisible. This is why the codebase carried a blanket "AvaloniaEdit is incompatible with
Semi.Avalonia" note for a long time.

**Cause:** AvaloniaEdit ships per-theme resource dictionaries keyed for Fluent and for Simple, and
Semi defines neither family. `docs/theme-audit-report.md` measures it: under bare Semi the Fluent
theme leaves **6** keys undefined and the Simple theme **9**. The distinction that matters is the
reference kind, not the count — a `DynamicResource` to a missing key paints an invisible control,
while a `StaticResource` **throws when the template loads**:

| Theme | Undefined under Semi | `StaticResource` (throws) |
|---|---:|---|
| AvaloniaEdit Fluent | 6 | `ContentControlThemeFontFamily`, `ControlContentThemeFontSize`, `ToolTipBorderThemeThickness` |
| AvaloniaEdit Simple | 9 | `ContentControlThemeFontFamily`, `FontSizeNormal`, `ThemeBackgroundBrush`, `ThemeBorderMidBrush`, `ThemeBorderThickness`, `ThemeForegroundColor` |

**Fix:** Nothing to do — `Resources/Compat/FluentKeys.Semi.axaml` and `SimpleKeys.Semi.axaml`,
merged in `App.axaml`, define every one of them, and the report scores both themes at **0**
undefined keys under "Semi + compat" across all six Semi variants. The blanket incompatibility
claim is therefore **stale**: it was accurate before those dictionaries landed and is not
accurate now.

Two things follow. Do not cite the old claim to reject AvaloniaEdit — re-read the report instead;
its "Undefined keys" table is regenerated by the `theme-audit` tool and is the authority. And if
the compat dictionaries are ever dropped from `App.axaml`, AvaloniaEdit is one of the hosts that
breaks **loudly** rather than subtly, because three of its missing keys are static.
