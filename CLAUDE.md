...
| **Driving the app through UI Automation for a retest** | [`scripts/retest/README.md`](./scripts/retest/README.md) — harnesses, not tests; nothing here runs in CI. ⛔ Read its **eight** lessons before hand-driving anything: a fixed sleep is not a measurement, `pwsh -File` does not parse PowerShell syntax so an array argument arrives as one string, `catch { continue }` without counting makes a degraded tree look empty, patterns never coordinate clicks, modals are `Window` elements **inside** the main window rather than siblings of it, a result announced only in a clearing status pill has to be measured by its effect, a needle matches the **name** and never the AutomationId, and `Invoke()` can return cleanly having done nothing |
