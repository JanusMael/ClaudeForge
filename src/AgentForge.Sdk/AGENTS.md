# AgentForge.Sdk — Agent Operational Guide
...
`AgentConfigClientCore(ConfigScope, SchemaRegistry?, preLoadedWorkspace?)` accepts
...
A TODO comment near the constructor in `AgentConfigClientCore.cs` marks the
...
`AgentConfigClientCore._cachedSchemaNodes` is:
...
**Async-method contract template.** `tests/AgentForge.Sdk.Tests/AgentConfigClientAsyncTests.cs`
