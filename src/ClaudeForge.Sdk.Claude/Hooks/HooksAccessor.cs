...
namespace Bennewitz.Ninja.ClaudeForge.Sdk.Claude.Hooks;
...
{
    // Typed as the Claude base rather than AgentConfigClientCore because
    // KnownEvents / KnownCommandTypes read the hook vocabulary off the Claude
    // settings schema, which only the Claude client knows how to name.
    private readonly ClaudeConfigClientBase _client;

    public HooksAccessor(ClaudeConfigClientBase client)
    {
        _client = client;
    }
