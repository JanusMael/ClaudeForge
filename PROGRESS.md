...
| **D6** — cut the app release | ◀ **NEXT, and the MAINTAINER'S** | The shipped artifact passes D3. ⭐ **Scope is written down**: [`docs/RELEASE-SCOPE.md`](docs/RELEASE-SCOPE.md) states what the tag produces, what ships, what is deliberately left out, and the three decisions that are the maintainer's. ⛔ **The release notes do NOT come from `CHANGELOG.md`** — `release.yml` reads the merged PR's body, else the tagged commit's message body, else auto-generated notes. This branch has no PR, so the curated `## [Unreleased]` section reaches the release only if it is put into the tagged commit's body or a PR |
