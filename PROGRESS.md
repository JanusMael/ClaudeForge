...
| ▶ RESUME | **`AgentForge.Core.Tests` next.** Converter rules for its and the later projects' unmapped forms are WIP in Templates **`68993d3`** (LOCAL ONLY, on `feat/mstest-to-xunit-rules` above `b4d3c4a`): `[Description]` → `[Trait("Description", …)]` (maintainer's choice, 2026-09-24), `nameof()` as a message, named `delta:`/`ignoreCase:`/`message:` made positional, the 4-argument comparison+message forms, `CollectionAssert.AreNotEqual`/`AllItemsAreUnique` with a message. ⏳ Still owed there: the helper overloads those rules call, fixtures, tests, then push to PR #1. Here: the sync `[Timeout]` in `GitignoreReaderTests` is made async BY HAND (`await Task.Run(…)`, so the timeout can actually fire) and canaried with a hang; `[Parallelize]` by hand as before. Per-commit gate: full build must SUCCEED first — a failed build lets `--no-build` run stale MSTest binaries and compare clean |
