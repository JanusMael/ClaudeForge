...
| Packaging | ⭐ The pack produces **exactly six** `.nupkg` — the five `AgentForge.*` and `JsonC` (eleven until plans/00005 moved `LayeredEditors.*` out) — ids prefixed `Bennewitz.Ninja.`, one version. ✅ **Latest published: `2026.3.925`** (2026-09-25, the BREAKING release; all six verified on the feed), and `SharedPackageVersion` consumes it. ⓘ This row named `2026.3.914` for three releases — the figure is not worth restating here, because `git ls-remote --tags origin 'packages-v*'` and `user/packages/nuget/<id>/versions` both answer it and neither goes stale |
...
1. ✅ **`packages-v2026.3.925` — the BREAKING release of the six — is PUBLISHED** (2026-09-25, run
   ...
   releasing is otherwise the maintainer's). All six verified on the feed. `SharedPackageVersion` is
   pinned to it in the same-day follow-up PR. ⭐ **Packaging direction, decided 2026-09-25:** `JsonC` moves
   to its own repository and nuget.org (needs a plan first); the five `AgentForge.*` stay here on the
   GitHub feed, released together — high churn, no consumer outside this repo, both products live here.
2. ✅ **`plans/00006` — MSTest → xUnit v3**, MERGED as #79 (`32abf46`, 2026-09-25), gate passed
...
| ▶ RESUME | ✅ **`plans/00006` is MERGED** — #79 → `main` as `32abf46`, 2026-09-25, admin, with #80 after it (`35348f7`); the only red jobs were `Feed Restore`/`Published Version`, every error NU1101 on the four `LayeredEditors.*` ids, and the merged tree's Architecture tests ran 77 / 0 failed / 3 skipped. The converter PRs Templates #1 (`0355106..6b23f21`), #2 (`63b7595`) and #4 (`56c08ac`) are merged, and `tests/Shared/MessageAssert.cs` is emitted from #4: each MSTest `Contains` family now keeps its own meaning (`Assert.*` asks the collection, `CollectionAssert.*` compares elements, via the new `ElementAssert`), proven by 108 MSTest-oracle pairs. This suite's 119 `Contains`/`DoesNotContain` sites, found by a compiler-backed scan of the MSTest tree, are all `CollectionAssert` over `List<string>`/`string[]`, so none changed verdict. Templates #3 added `scripts/mstest-areequal-scan.cs`, the drift-8 scan as a reusable tool. Templates #2 fixed a weakening its review found: the generic `OrdinalAssert.Contains`/`DoesNotContain` handed a dictionary's `Keys` to xUnit, which compares non-sets by the default equality, so `IsFalse(keys.Contains("A"))` over case-insensitive keys passed after conversion where MSTest failed (sets were never affected: xUnit 3.2.2 asks an `ISet<T>` for itself). **The suite's 10 `OrdinalAssert.DoesNotContain` sites were read:** 8 bind the ordinal string overload, and the 2 generic ones pass `BashCommandSplitter.ReadOnlyCommandNames` (a `FrozenSet` built with `StringComparer.Ordinal`, which IS the default string equality) and a `List<string>` — so no verdict here changed. #80 retired `docs/AVALONIA-GOTCHAS.md` for XamlQuality's living copy and states the inflow rule in `AGENTS.md` §7. Follow-ups, none blocking: (1) ✅ the compilation-backed scan for collection-typed `AreEqual` sites found none (drift 8; #81); (2) ✅ `xUnit1051` is ENFORCED — all 469 sites given `TestContext.Current.CancellationToken` by Templates' `scripts/xunit1051-fix.cs` (xUnit's fixer cannot Fix All, so `dotnet format` changed nothing), 146 of them as `ct:` because an optional parameter precedes the token; one line per site, build 0 warnings, three whole-suite runs 3,295 / 0 failed; (3) ✅ `ConfigScopeAdapter.ToConfigScope`'s id ambiguity across ladders — the id fallback is removed (drift 10); (4) ✅ the `ReloadHardeningTests` timing flake — captured and fixed (drift 9): two background reads of `settings.json` shared READ only; 0 of 150 under load after, ~25% before. ⚠ Pushing Templates from here needs `-c credential.helper= -c "credential.helper=!gh auth git-credential"` |
...
8. ⛔⛔ **The first four conversions WEAKENED every string assertion, and every gate was green.**
   ...
   ✅ **Scanned 2026-09-25 — NO site is affected.** A Roslyn analyzer, injected into a build of the
   MSTest tree (`d0f3ea7`) through `CustomAfterMicrosoftCommonTargets` so no scanned file changed,
   classified every `Assert.AreEqual`/`AreNotEqual` by its bound parameter type AND each argument's
   own static type: **2,935 sites — 2,891 scalar, 44 `object`-typed, 0 collection-typed.** Each of the
   44 has one side statically a scalar (`string`, `int`, `long`, `double`, an enum), and MSTest passed
   them via `expected.Equals(actual)`, so the other side was an equal scalar at runtime. Coverage:
   a text count finds 2,936, the one extra being a doc comment in `SampleHeadlessTests.cs`.
   Canaried: nine known cases (`List<T>`, `int[]`, `JsonArray`, `AreEqual<object>(list, …)` and
   `AreNotEqual` → collection-by-reference; `ImmutableArray` → overrides `Equals`; `object` → opaque;
   `string`/`int` → scalar; `CollectionAssert` not reported) classified exactly as predicted.
9. ⛔ **xUnit v3 RANDOMISES test order per run** (`--seed`); MSTest always ran one fixed order. A suite
   ...
   **no order-dependent failure**. Randomisation is KEPT — it is what found drift 10 — and a failure
   ...
   under seed 3, which then passed twice under the same seed — TIMING, not order. **Captured and
   fixed 2026-09-25**, and it was NOT the `VerifyAccess` flake: 12 whole-assembly seeds ran clean,
   so the class was run 150 times with every core kept busy, full output kept per failure. 10 of
   the first ~40 runs failed, every one `IOException: … settings.json … being used by another
   process` from the TEST's own `File.WriteAllTextAsync` (line 196) while the reload started on the
   line before was reading. ⚠ **The first fix was not the fix.** Making `ConfigFileLoader` share
   write access left the rate unchanged (40 of 150), and a clean rebuild ruled out a stale binary.
   Two probes then ruled out another process: 3,000 back-to-back rewrites of one file, and 1,000
   fresh sandboxes rewritten four times, both under load, met **0** violations. The holder was
   named by a first-chance capture — a thread briefly holding `settings.json` exclusively, and an
   `AppDomain.FirstChanceException` hook recording the stack of every other opener. ⛔ Its first two
   runs "found nothing" because the exclusion filter matched the handler's OWN frame; a canary
   that expected the loader's stacks caught it. The real holder: **`UserMemoryService.ReadFirstNonEmptyLine`**,
   the memory editor's background subtitle read, which runs on every refresh after a reload,
   reaches `settings.json`, and opened with `FileShare.Read` — while its twin
   `EditableMemoryService.LoadDescription` already shared write, with a comment naming exactly this
   case. Both reads now share `ReadWrite | Delete`; the loader's is kept on principle (a writer and
   a default reader looping on one file for 5 s: 28,987 writer failures; with the shared read, 0).
   **With both: 0 of 150** under the same load. Guards, each canaried red with the old read:
   `ConfigFileLoadFailureTests.AFileHeldOpenByAWriter_StillLoads` and
   `UserMemoryServiceTests.Subtitle_StillReads_WhileAWriterHoldsTheFile` (the deterministic mirror
   direction; the sharing check is symmetric).
10. ⛔ **`ConfigScopeAdapterTests.ToConfigScope_ResolvesRealWrappersAndForeignScopesAlike` only ever
    ...
    it the test fails 8/8, with it passes 8/8. ✅ **The ambiguity is gone, 2026-09-25 — removed, not
    guarded.** Measured first: `ConfigScopeAdapter` is the ONLY `IEditorScope` in every first-party
    assembly, and the ScopedEditors packages define none (a metadata scan, canaried by finding the
    adapter itself), so the library can only hand back scopes the host created and the id fallback
    served test fakes alone. `ToConfigScope` now refuses anything that is not a `ConfigScopeAdapter`,
    and the reset seam `ForgetNonDefaultLaddersForTesting` is deleted with its constructor call.
    `ToConfigScope_RefusesAForeignScope_EvenOneNamingARealRung` guards it (canaried: an id fallback
    put back fails exactly it); 8 seeds of `ClaudeForge.Tests` pass without the seam. ⚠ When
    OpenCodeForge rejoins, its copy of the old `ToConfigScope_ResolvesRealWrappersAndForeignScopesAlike`
    (parked branch, `ConfigScopeAdapterTests.cs:99`) asserts the removed fallback and must take the
    new contract.
