...
| Trim check | ✅ **6/6 six-RID ONE-app matrix, zero IL diagnostics, 2026-09-17 at `1e41f87`** — linux-x64/arm64, win-x64/arm64, osx-x64/arm64. ⛔ **The six-RID matrix has NOT been re-run since, and A5 did not re-cover it.** This row previously said *"A5 is what re-covers this before the tag"*; A5 ran on 2026-09-18 at `d2fe2e9` and publishes **win-x64 only**, so five of the six RIDs are still covered by the 2026-09-17 run plus a per-commit linux-x64 publish. ⚠ The gap is macOS and the two arm64 targets. **Phase D's `D3` is where a shipped artifact is measured**; if a full six-RID re-run is wanted before the tag, it is a separate ~10-minute step and nothing currently schedules it. ⚠ **6/6, not 12/12**, because the release branch ships one app; the twelve-publish figure below is the parked branch's |
...
### ✅ A5 PASSED — 2026-09-18, on `d2fe2e9`, the commit being tagged

Packed at `0.0.0-local-20260918110913`; eleven packages; full **Release** suite in package mode at
