...
| Merged from `main` | ✅ **Integrated by hand on 2026-09-16**, after `main` released **`v2026.3.916`**. ⭐ **Use `git cherry HEAD origin/main`, never the commit count.** The count reads **55 behind** and that is mostly an artefact: the 2026-09-16 history rewrite gave every already-merged `main` commit a new SHA here, so git stops recognising the originals. Patch-ids see through it — `git cherry` reported **27 already present by content, 6 genuinely new**, and those 6 are what was integrated. ⛔ **A `git merge origin/main` is still the wrong tool**: the merge base is `3c7aaab` (2026-09-01), `main`'s paths no longer exist here (`ClaudeForge.Sdk`→`AgentForge.Sdk`, the backup VM moved into `AgentForge.Avalonia.Shell`), and this repo has shipped two duplicate-attribute defects from *clean* auto-merges. Ports go through `AGENTS.md` §*Hand-porting a fix*. ⓘ What the 6 turned out to be: **2 ported** (YAML front-matter, winget rename), **1 ported as a union** (deep links), **1 already present by a different mechanism** (tab a11y — this branch names containers via `ToString()`, `main` via a style; porting would have added a redundant second mechanism), **1 doc** (README), **1 reconciled** (CHANGELOG) |
