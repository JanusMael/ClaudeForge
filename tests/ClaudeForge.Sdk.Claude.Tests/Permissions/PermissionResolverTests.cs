...
public sealed class PermissionResolverTests
{
    ...
    public void Allow_WhenOnlyAllowMatches()
    {
        ...
        Assert.Equal(PermissionOutcome.Allow, d.Outcome);
        Assert.Equal(PermissionBucket.Allow, d.MatchedBucket);
        Assert.Equal("Bash(npm *)", d.MatchedRule!.Value);
    }
    ...
    public void Deny_BeatsAllow_EvenWhenBothMatch()
    {
        ...
        Assert.Equal("Bash(git push *)", d.MatchedRule!.Value);
    }
    ...
    public void Read_DenySubHierarchyUnderAllowRoot_DenyWins()
    {
        ...
        Assert.Equal(PermissionOutcome.Allow, underRoot.Outcome);

        PermissionDecision underPrivate = Resolve(
            PermissionCandidate.Read(@"C:\c\cl\private\secret.txt"),
            allow: ["Read(//C:/c/cl/**)"],
            deny: ["Read(//C:/c/cl/private/**)"]);
        Assert.Equal(PermissionOutcome.Deny, underPrivate.Outcome);
        Assert.Equal("Read(//C:/c/cl/private/**)", underPrivate.MatchedRule!.Value);
    }
    ...
    public void Default_WhenNoRuleMatches()
    {
        ...
        Assert.Equal(PermissionOutcome.Default, d.Outcome);
        Assert.Null(d.MatchedRule);
        Assert.Equal(PermissionDefaultMode.AcceptEdits, d.DefaultMode);
    }
    ...
    public void Compound_NotAllowed_WhenOneSubcommandUnmatched()
    {
        ...
        PermissionDecision d = Resolve(
            ...
            allow: ["Bash(npm *)"]);
        Assert.NotEqual(PermissionOutcome.Allow, d.Outcome);
        Assert.Equal(PermissionOutcome.Default, d.Outcome);
    ...
    public void Merged_DenyInHigherScope_BeatsAllowInLowerScope()
    {
        ...
        Assert.Equal(PermissionOutcome.Deny, d.Outcome);
        Assert.Equal(ConfigScope.Managed, d.MatchedScope);
    }
    ...
    public void Merged_AllowAttributedToHighestPrecedenceScope()
    {
        ...
        Assert.Equal(PermissionOutcome.Allow, d.Outcome);
        Assert.Equal(ConfigScope.Project, d.MatchedScope);
    }
    ...
    public void Merged_DenyCheckedAcrossAllScopes_BeforeAnyAllow()
    {
        ...
        Assert.Equal(PermissionOutcome.Deny, d.Outcome);
        Assert.Equal(ConfigScope.User, d.MatchedScope);
    }
    ...
    public void Compound_PowerShell_OneSubDenied_Denies()
    {
        ...
        Assert.Equal(PermissionOutcome.Deny, d.Outcome);
        OrdinalAssert.Contains("Remove-Item", d.DecidingSubcommand!);
    }
