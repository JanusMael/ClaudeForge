...
public class SearchViewModelTests
{
    ...
    {
        Assert.Equal(string.Empty, SearchViewModel.BuildSnippet(string.Empty, "x", 50));
    }
    ...
    public void BuildSnippet_NoMatch_TruncatesToLeadingMaxLen()
    {
        ...
        Assert.True(snip.EndsWith("…"), "Truncated no-match snippet must end with ellipsis.");
        MessageAssert.Equal(50 + 1, snip.Length, "Length = maxLen + ellipsis char.");
    }
    ...
    public void BuildSnippet_NoMatch_ShortText_ReturnsAsIs()
    {
        ...
        MessageAssert.Equal("short", snip, "When text fits in maxLen, no truncation or ellipsis.");
    }
    ...
    public void BuildSnippet_MatchInMiddle_PadsBothSides()
    {
        ...
        Assert.True(snip.StartsWith("…"), $"Expected leading ellipsis, got: {snip}");
        Assert.True(snip.EndsWith("…"), $"Expected trailing ellipsis, got: {snip}");
        OrdinalAssert.Contains("needle", snip);
    }
    ...
    public void BuildSnippet_CaseInsensitiveMatch()
    {
        ...
        OrdinalAssert.Contains("Claude", snip);
    }
    ...
    public void ExecuteSearch_DangerouslySkip_AddsSyntheticRow_WhenPermissionsNodePresent()
    {
        ...
        MessageAssert.Equal(1, vm.SearchResults.Count, "Synthetic --dangerouslySkipPermissions row should appear.");
        Assert.True(vm.SearchResults[0].IsSynthetic);
        Assert.Equal("--dangerouslySkipPermissions", vm.SearchResults[0].PropertyDisplayName);
        MessageAssert.Same(permNode, vm.SearchResults[0].Node,
            "Synthetic row must navigate to the Permissions node when present.");
        Assert.True(vm.IsSearchOpen, "Popup should open whenever results are produced.");
    }
    ...
    public void ExecuteSearch_QueryTooShort_OmitsSyntheticRow()
    {
        ...
        MessageAssert.Equal(0, vm.SearchResults.Count,
    ...
    public void ExecuteSearch_SpecializedEditor_MatchedByPageTitle()
    {
        ...
        Assert.True(vm.SearchResults.Any(r => r.Node == permNode),
        ...
        Assert.False(vm.SearchResults.Any(r => r.Node == permNode),
    ...
    {
        Assert.Equal("permissions.allow",
    ...
    {
        Assert.Equal("\"", SearchViewModel.StripPhraseQuotes("\""));
        Assert.Equal("", SearchViewModel.StripPhraseQuotes(""));
    }
    ...
    public void ExecuteSearch_QuotedQuery_StripsQuotesBeforeMatching()
    {
        ...
        IReadOnlyList<SchemaSearchResult> FakeSearch(string query)
        {
            ...
            Assert.False(query.StartsWith('"'), "Query reaching SDK must have quotes stripped.");
            return query.Contains("allow", StringComparison.OrdinalIgnoreCase)
                ?
                [
                    new SchemaSearchResult(
    ...
    public void ExecuteSearch_DottedJsonPath_MatchesSchemaNode()
    {
        ...
        Assert.True(vm.SearchResults.Count > 0,
        ...
        Assert.False(vm.SearchResults.Any(r => r.PropertyKey == "permissions.deny"),
    ...
    public void ExecuteSearch_PartialDottedPath_MatchesBothAllowAndDeny()
    {
        ...
        MessageAssert.Contains("permissions.allow", keys, "allow node must match.");
        MessageAssert.Contains("permissions.deny", keys, "deny node must match.");
        Assert.False(keys.Contains("model"), "'model' must not appear in a 'permissions' search.");
    }
    ...
    public void EssentialsTriggers_TableCovers_EveryCardId()
    {
        ...
        {
            MessageAssert.Contains(triggerKey, ids.ToList(),
                $"Trigger key '{triggerKey}' must reference a real card id.");
        }

        foreach (string id in ids)
        {
            Assert.True(ClaudeSyntheticSearch.EssentialsTriggers.ContainsKey(id),
    ...
    public void ExecuteSearch_EssentialsTriggers_ProduceSyntheticHit_Thinking()
    {
        ...
        Assert.True(vm.SearchResults.Any(r =>
                r.IsSynthetic
    ...
    public void ExecuteSearch_EssentialsTriggers_PartialMatch_Sandbox()
    {
        ...
        List<string> hits = vm.SearchResults
                              ...
                              .ToList();
        Assert.Contains(EssentialsViewModel.CardIdSandboxEnabled, hits);
        Assert.Contains(EssentialsViewModel.CardIdSandboxDomains, hits);
    }
    ...
    public void ExecuteSearch_EssentialsTriggers_QueryTooShort_NoHits()
    {
        ...
        Assert.False(vm.SearchResults.Any(r => r.IsSynthetic && r.PropertyKey.Length > 0),
