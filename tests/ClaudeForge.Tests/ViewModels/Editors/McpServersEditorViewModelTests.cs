...
// ⓘ xUnit1031 (no blocking on a task) is suppressed in this file: one synchronous test opens its
// client with GetAwaiter().GetResult(), as it did under MSTest. Making it async would rewrite the
// test, which the xUnit move does not do (plans/00006 decision 5).
...
public class McpServersEditorViewModelTests
{
    ...
    public void AddServer_AddsToCollection()
    {
        ...
        Assert.Equal("my-server", vm.Servers[0].Name);
        Assert.Equal("my-server", vm.SelectedServer?.Name);
    }
    ...
    public void RemoveServer_RemovesAndUpdateSelection()
    {
        ...
        Assert.Single(vm.Servers);
        Assert.Equal("b", vm.Servers[0].Name);
    }
    ...
    public void LoadFromLayered_PopulatesServers()
    {
        ...
        Assert.Single(vm.Servers);
        Assert.Equal("context7", vm.Servers[0].Name);
        Assert.Equal("npx", vm.Servers[0].Command);
        Assert.Equal(2, vm.Servers[0].Args.Count);
    }
    ...
    public void ToJsonValue_RoundTrips()
    {
        ...
        Assert.NotNull(json);
        Assert.NotNull(json!["myserver"]);
        JsonObject? srvJson = json["myserver"] as JsonObject;
        Assert.Equal("node", srvJson!["command"]!.GetValue<string>());
        Assert.Single((srvJson["args"] as JsonArray)!);
    }
    ...
    public void LoadFromLayered_PicksFirstServer_OnFirstLoad()
    {
        ...
        MessageAssert.Equal("alpha", vm.SelectedServer?.Name,
    ...
    public void OtherScopesWithData_DedupesEntriesAtSameScope()
    {
        ...
        MessageAssert.Equal(1, vm.OtherScopesWithData.Count,
            "Two Managed-scope entries must collapse into a single Managed badge.");
        Assert.Equal("managed", vm.OtherScopesWithData[0].Id);
    }
    ...
    public void OtherScopesWithData_RefreshedAfterReset()
    {
        ...
        vm.AddServerCommand.Execute(null);
        MessageAssert.Equal(2, vm.Servers.Count, "Setup: both loaded and unsaved server present");

        // Reset → unsaved server is gone; OtherScopesWithData must still show Project badge.
        vm.ResetToInheritedCommand.Execute(null);

        MessageAssert.Equal(1, vm.Servers.Count, "Reset should remove unsaved server");
        MessageAssert.Equal(1, vm.OtherScopesWithData.Count,
    ...
    public void RemoveAllServers_DoesNotClearIsModified()
    {
        ...
        McpServersEditorViewModel vm = new(McpSchema(), ConfigScope.User);
        vm.LoadFromLayered(LayeredWithServers(ConfigScope.User, obj), ConfigScope.User);
        MessageAssert.Equal(1, vm.Servers.Count, "Setup: one server loaded");

        vm.RemoveServerCommand.Execute(vm.Servers[0]);

        MessageAssert.Equal(0, vm.Servers.Count, "Server should be removed");
        Assert.True(vm.IsModified,
    ...
    public void RemoveServerAfterLoad_FiresIsModifiedPropertyChanged()
    {
        ...
        McpServersEditorViewModel vm = new(McpSchema(), ConfigScope.User);
        vm.LoadFromLayered(LayeredWithServers(ConfigScope.User, obj), ConfigScope.User);
        Assert.True(vm.IsModified, "Setup: IsModified is already true after load.");
    ...
    public void EditingArgValueOnLoadedServer_FiresIsModifiedPropertyChanged()
    {
        ...
        McpServersEditorViewModel vm = new(McpSchema(), ConfigScope.User);
        vm.LoadFromLayered(LayeredWithServers(ConfigScope.User, obj), ConfigScope.User);
        Assert.True(vm.Servers[0].Args.Count >= 1, "Setup: at least one Arg loaded.");
    ...
    public async Task LoadFromLayered_WithSdkClient_ReadsThroughTypedAccessor()
    {
        ...
        try
        {
            ...
            MessageAssert.Equal(1, vm.Servers.Count, "SDK path should yield exactly the SDK-set server.");
            Assert.Equal("from-sdk", vm.Servers[0].Name);
            Assert.Equal("stdio", vm.Servers[0].Type);
            Assert.Equal("/usr/bin/node", vm.Servers[0].Command);
            Assert.Equal(2, vm.Servers[0].Args.Count);
            Assert.Equal("main.js", vm.Servers[0].Args[0].Value);
        }
        finally
        {
    ...
    public void LoadFromLayered_WithoutSdkClient_FallsBackToLegacyJsonPath()
    {
        ...
        Assert.Single(vm.Servers);
        Assert.Equal("legacy", vm.Servers[0].Name);
        Assert.Equal("echo", vm.Servers[0].Command);
        Assert.Single(vm.Servers[0].Args);
        Assert.Equal("hello", vm.Servers[0].Args[0].Value);
    }
    ...
    public void FormatTransport_StreamableHttp_RendersAs_Http_ForCombobox()
    {
        ...
        try
        {
            ...
            Assert.Single(vm.Servers);
            MessageAssert.Equal("http", vm.Servers[0].Type,
                "StreamableHttp must surface as 'http' for the editor's ComboBox.");
            Assert.Equal("https://api.example.com", vm.Servers[0].Url);
        }
        finally
        {
            PlatformPaths.TestUserProfileOverride = previousOverride;
    ...
    public void DeleteAfterLoad_FiresIsModified_ForceFireContract()
    {
        ...
        Assert.True(vm.IsModified, "Precondition: load with two servers flags IsModified=true.");
