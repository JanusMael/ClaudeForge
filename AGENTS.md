...
| **`ConfigScope` enum value order ↔ `ClaudeScope._cache` array order**. `ClaudeScope.For(ConfigScope)` indexes `_cache` by `(int)scope`. The 4 entries in `_cache` MUST appear in `ConfigScope` numeric order. Reorder one → reorder the other. | `For(ConfigScope.User)` returns the wrapper for a different scope → permission checks pass against the wrong scope, silent data corruption. | Enum: `src/AgentForge.Core/Settings/ConfigScope.cs` (Managed=0, Local=1, Project=2, User=3). Cache: `src/ClaudeForge/Adapters/ClaudeScope.cs`. Sidecar: [`src/AgentForge.Core/Settings/AGENTS.md`](./src/AgentForge.Core/Settings/AGENTS.md). |
