...
- [ ] Add the key to `src/ClaudeForge/Localization/Strings.resx`.
...
- [ ] Bind from AXAML via `{x:Static loc:Strings.KeyName}` (the `loc:` namespace is already declared in existing views; copy from any existing AXAML).
