...
- [ ] `Description` in the csproj. This repo uses it as the project's design rationale, not a package blurb — see `src/AgentForge.Jsonc/AgentForge.Jsonc.csproj` for the tone.
...
- [ ] **State `IsPackable` explicitly**, `true` or `false`, once plan 00001 item 2 lands. Eleven shared projects are published as private NuGet packages; a project that says nothing is packaged, or skipped, by being forgotten. ⚠ Until that item ships there is nothing to state — check whether `src/Directory.Build.props` carries the packaging block before adding a line that nothing reads.
